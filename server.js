...
function classify(question) { for (const [pattern, reason] of alwaysEscalate) if (pattern.test(question)) return { escalate: true, reason, category: 'sensitive' }; return null; }
...
function deterministicAnswer(question, rows) {
  ...
  return { answer: rows[0].content.length > 420 ? `${rows[0].content.slice(0, 417)}...` : rows[0].content, confidence: Math.min(.94, Math.max(.75, Number(rows[0].score) + .75)), source: rows[0].title };
}

app.post('/api/auth/register', async (req, res) => {
  const { email, password, fullName, role = 'employee', team } = req.body;
  if (!email || !password || !fullName || password.length < 8 || !allowedRoles.has(role)) return res.status(400).json({ error: 'Email, full name, valid role, and an 8-character password are required' });
  try {
    const hash = await bcrypt.hash(password, 12);
    const { rows } = await pool.query('INSERT INTO users(email,password_hash,full_name,role,team) VALUES($1,$2,$3,$4,$5) RETURNING id,email,full_name,role,team', [email.toLowerCase(), hash, fullName, role, team || null]);
...
app.post('/api/conversations/:id/messages', auth, async (req, res) => {
  ...
  const sensitive = classify(question); let decision = sensitive ? { escalate: true, reason: sensitive.reason, category: sensitive.category, confidence: null } : null;
  const rows = decision ? [] : await retrieve(question, req.user.role === 'it_support' ? 'it' : 'all');
  const grounded = decision ? null : deterministicAnswer(question, rows);
